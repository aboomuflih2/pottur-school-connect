...
const GalleryManager = () => {
    ...
    const { data: photos = [], isLoading } = useQuery<GalleryPhoto[], Error>({
        queryKey: ['galleryPhotos'],
        queryFn: async () => {
            // @ts-ignore
            const response = await djangoAPI.getGalleryPhotos();
            // @ts-ignore
    ...
    const createMutation = useMutation({
        mutationFn: (photoData: any) => djangoAPI.createGalleryPhoto(photoData),
        ...mutationOptions,
    });

    const updateMutation = useMutation({
        mutationFn: (photoData: any) => djangoAPI.updateGalleryPhoto(editingPhoto!.id, photoData),
        ...mutationOptions,
    });

    const deleteMutation = useMutation({
        mutationFn: (photoId: string) => djangoAPI.deleteGalleryPhoto(photoId),
        ...mutationOptions,
    });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (editingPhoto) {
            updateMutation.mutate(photoData);
        } else {
      ...
      const toggleActiveStatus = async (photo: GalleryPhoto) => {
        updateMutation.mutate({ ...photo, is_active: !photo.is_active });
      };
    ...
    const resetForm = () => {
      ...
      const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        try {
          ...
          toast({ title: 'File uploaded', description: 'Image uploaded successfully.' });
        } catch (err) {
